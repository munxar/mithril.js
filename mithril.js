...
function factory(window) {
    ...
    function forOwn(obj, f) {
        if (obj) {
            Object.keys(obj).some(function (prop) { return f(obj[prop], prop); });
        }
    }
    var parser = /(?:(^|#|\.)([^#\.\[\]]+))|(\[.+?])/g;
    var attrParser = /\[(.+?)(?:=("|'|)(.*?)\2)?]/;
    ...
    function checkForAttrs(pairs) {
        return pairs != null &&
            isObject(pairs) && !("tag" in pairs || "view" in pairs || "subtree" in pairs);
    }
     ...
     * {...mNode[]} Zero or more Mithril child nodes. Can be an array,
     *                      or splat (optional)
     */
    var m = function (tag, pairs) {
        for (var args = [], i = 1; i < arguments.length; i++) {
            args[i - 1] = arguments[i];
        }
        if (isObject(tag)) {
            return parameterize(tag, args);
        }
    ...
    function handleTextNode(cached, data, index, parent, shouldReattach, editable, parentTag) {
        if (cached.nodes.length === 0) {
            return handleNonexistentNodes(data, parent, index);
        }
        else if (cached.valueOf() !== data.valueOf() || shouldReattach) {
            return reattachNodes(data, cached, parent, editable, index, parentTag);
        }
        else {
            cached.nodes.intact = true;
            return cached;
    ...
    function buildArray(data, cached, parentElement, index, parentTag, shouldReattach, editable, namespace, configs) {
        ...
        if (!intact)
            diffArray(data, cached, nodes);
        return cached;
    }
    function makeCache(data, cached, index, parentIndex, parentCache) {
        if (cached != null) {
            if (type.call(cached) === type.call(data))
                return cached;
    ...
    function initObjectNode(isNew, data, namespace, hasKeys, cached, editable, configs, views, controllers, parentElement, shouldReattach, index) {
        var node;
        if (isNew) {
    ...
    function buildObject(data, cached, editable, parentElement, index, shouldReattach, namespace, configs) {
        ...
        if (isString(data.tag)) {
            return initObjectNode(cached.nodes.length === 0, data, getObjectNamespace(data, namespace), hasKeys, cached, editable, configs, views, controllers, parentElement, shouldReattach, index);
        }
    }
    ...
    function trySetAttribute(attr, dataAttr, cachedAttr, node, namespace, tag) {
        ...
        if (attr === "config" || attr === "key")
            ...
            if (attr === "href") {
                ...
                node.setAttribute(attr === "className" ? "class" : attr, dataAttr);
            }
        }
    ...
    function parameterize(component, args) {
        ...
        var output = { controller: Ctrl, view: view, attrs: {} };
    ...
    function redraw() {
        ...
        lastRedrawId = null;
        lastRedrawCallTime = new Date().getTime();
        m.redraw.strategy("diff");
    }
    ...
    function computeAndLaunchRedirect(shouldReplaceHistoryEntry) {
        if (window.history.pushState) {
            ...
            computePostRedrawHook = function () {
                window.history[method](null, $document.title, modes[m.route.mode] + currentRoute);
            };
    ...
    m.route = function (root, arg1, arg2, vdom) {
        if (arguments.length === 0) {
            ...
            runHistoryListener(m.route.mode === "hash" ? "onhashchange" : "onpopstate");
        }
    ...
    m.request = function (options) {
        ...
        options.onload = options.onerror = function (ev) {
            ...
            try {
                response = (unwrap || identity)(deserialize(extract(ev.target, options)), ev.target);
                if (doSuccess) {
