...
function factory(window) {
    ...
    function forEach(list, f) {
        list.some((item, index) => f(item, index));
    }

    function forOwn(obj, f) {
        if (obj) {
            Object.keys(obj).some(prop => f(obj[prop], prop));
        }
    }
     ...
     *                      or splat (optional)
     */
    var m = <IMithrilStatic>function (tag, pairs) {
    ...
    function handleNonexistentNodes(data:IData, parent, index) {
        var nodes;
        if (data.$trusted) {
